...
로컬 프로젝트의 변경사항을 원격 저장소에 푸시하고, `jma` 원격 서버에 접속하여 최신 코드를 pull하고, fQRGen을 재시작한 뒤 `tmux`의 `qa` 세션을 통해 일괄 테스트(Batch Test)를 실행하는 자동화 워크플로우입니다.
...
## 2. JMA 원격 서버: 환경 갱신 및 fQRGen 재시작
...
```bash
ssh jma '/opt/homebrew/bin/tmux send-keys -t qa "cd /Users/nowage/_git/__all/fQRGen && git pull && pkill -f \"MacOS/fQRGen\" || true && sh _tool/run.sh" C-m'
```
